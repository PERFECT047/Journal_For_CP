	...
	while(tc--){
		...
		for(ll i=0;i<n;i++){
			...
			for(int pos=0;pos<3;pos++){
				...
				for(char a:s) if(a==c) tick = 1;
				
				if(tick) s_mask += (1<<pos);
			}
			f[i+1][s_mask] = min(f[i+1][s_mask],cost);
			for(ll mask =0;mask<8;mask++){
				f[i+1][mask] = min(f[i+1][mask],f[i][mask]);
				f[i+1][mask|s_mask] = min(f[i][mask]+cost,f[i+1][mask|s_mask]);
			}
